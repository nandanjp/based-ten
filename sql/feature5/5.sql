WITH YourRankedItems AS (
        ...
        WHERE l.username = 'JaneSmith'
    ),
    SimilarLists AS (
        SELECT l.username, l.listName, COUNT(lk.likerusername) AS likes
        FROM Lists l JOIN Likes lk ON l.username = lk.likingusername
    ...
    RecommendedLists AS (
        SELECT sl.ususername, sl.listName, sl.likes, COUNT(li.itemID) AS similarity
