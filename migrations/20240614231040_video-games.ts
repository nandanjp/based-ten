...
export async function up(knex: Knex): Promise<void> {
  const createTable = path.join(__dirname, '../sql/video-games/createTables.sql')
  const create = await promises.readFile(createTable, 'utf-8')
  const populateTable = path.join(__dirname, '../sql/video-games/populateTables.sql')
...
export async function down(knex: Knex): Promise<void> {
  const dropTables = path.join(__dirname, '../sql/video-games/dropTables.sql')
