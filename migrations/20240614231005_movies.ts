...
export async function up(knex: Knex): Promise<void> {
  const createTable = path.join(__dirname, '../sql/movies/createTables.sql')
  const create = await promises.readFile(createTable, 'utf-8')
  const populateTable = path.join(__dirname, '../sql/movies/populateTables.sql')
...
export async function down(knex: Knex): Promise<void> {
  const dropTables = path.join(__dirname, '../sql/movies/dropTables.sql')
