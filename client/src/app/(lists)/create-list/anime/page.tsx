"use client";
import { createList, getAnimeById, getAnime, getVideoGameById } from "@/app/actions";
...
const CreateListPage = ({
  ...
  const {
    ...
    queryKey: ["get-anime-by-id"],
    queryFn: async () => {
      return await getAnimeById(firstItemId);
    },
  });
  const {
    data: all,
      ...
      {isSingleError || isSingleFetching || isAllError || isAllFetching ? (
        ...
        <div className="flex flex-col justify-center items-center gap-4 p-4 min-w-full">
          {selectedItems.map((item, index) => (
            <AddListItem
              listItem={item}
              list={all?.response.map((item) => ({ createdon: item.createdon, id: item.id, mediaimage: item.mediaimage, title: item.title, type: "anime" }))}
